...
/** Concurrency per model on each API key: 2 requests per model slot.
 * Ensures strict safety within 15 RPM free-tier limit while keeping high throughput across all keys.
 * 2 models × 2 requests = 4 simultaneous requests per key. */
const VERIFY_CONCURRENCY_PER_MODEL = 2
...
class Scheduler {
  ...
  private startChunkUpload(job: Job, lane: KeyLane, chunkIndex: number, prefetch: boolean): Promise<{ uri: string; name: string }> {
    ...
    p = (async () => {
      ...
      const hasActiveModel = CHUNK_MODEL_POOL.some((m) => getModelUsage(m.id, lane.apiKey) < (m.rpd || 20))
      if (!hasActiveModel) {
  ...
  private prefetchNextChunks(job: Job, lane: KeyLane) {
    ...
    const hasActiveModel = CHUNK_MODEL_POOL.some((m) => {
      const isExhausted = getModelUsage(m.id, lane.apiKey) >= (m.rpd || 20)
      ...
      try {
        ...
        if (e.kind === 'invalid_key') {
          ...
          if (used >= rpdCap) {
            ...
            job.verifyQueue.push(gi)
            addLog(scan, 'error', `Verifier: ${displayModelName(m.id)} (key ${lane.idx}) daily quota limit reached (${used}/${rpdCap} RPD) — group ${g.id} re-queued for another key`)
          } else {
            ...
            const coolDuration = outcome.waitSec * 1000 || RATE_COOLDOWN_MS
            const coolUntil = Date.now() + coolDuration
            ...
            addLog(
              scan,
              'error',
              `[Verifier Quota / TPM Hit] Key ${lane.idx} · ${displayModelName(m.id)}: ${e.message.slice(0, 140)} — TPM hit hua he! Cooldown ${outcome.waitSec}s wait kar rahe hain (Daily Quota remaining: ${used}/${rpdCap} RPD bacha hua hai). Group ${g.id} re-queued; wait karo...`,
            )
          }
        } else {
          g.attempts += 1
      ...
      try {
        ...
        if (e.kind === 'rate' || e.kind === 'rpd') {
          ...
          if (outcome.action === 'exhausted') {
            ...
            job.cooldownUntil[pk] = Date.now() + (outcome.waitSec * 1000 || RATE_COOLDOWN_MS)
            st.state = 'cooling'
          }
        }
        throw err
  ...
  private async worker(job: Job, lane: KeyLane, m: ModelSpec) {
    ...
    while (true) {
      ...
      // Fast RPD check: strictly check against Settings Quota
      const usedNow = getModelUsage(m.id, lane.apiKey)
      const rpdCapNow = m.rpd || 20
      if (usedNow >= rpdCapNow) {
        if (st.state !== 'exhausted') {
          st.state = 'exhausted'
          st.currentChunk = null
          addLog(scan, 'warn', `${displayModelName(m.id)} (key ${lane.idx}) daily quota reached (${usedNow}/${rpdCapNow} RPD) — removed from pool`)
          this.mark(job)
        }
        return
      } else if (st.state === 'exhausted') {
      ...
      // ATOMIC GLOBAL COORDINATOR ACQUISITION:
      // Try to acquire this (key × model) lane right now across all parallel scans.
      // If it is busy in another scan, in 70s cooldown, in pacing, or exhausted:
      // DO NOT pull any chunk! Leave chunks in job.queue so other free workers can grab them immediately.
      const acquired = globalGeminiCoordinator.tryAcquireLane({
        scanId: scan.id,
      ...
      // Lock is ATOMICALLY held exclusively for this scan! Now pull next chunk.
      ...
      let releaseGlobalLock: ((sec?: number, cooldownOverrideMs?: number) => void) | null = acquired.release
      try {
        ...
        if (isPolicyBlocked) {
          ...
          if (used >= rpdCap) {
            ...
            const coolDuration = quotaOutcome.waitSec * 1000 || CHUNK_COOLDOWN_MS
            job.cooldownUntil[this.rateKey(lane, m)] = Date.now() + coolDuration
            const laneState = job.scan.keyLanes?.find((l) => l.idx === lane.idx)
            if (laneState) {
              const ms = laneState.models.find((item) => item.id === m.id)
              if (ms) {
                ms.state = 'cooling'
                ms.cooldownUntil = Date.now() + coolDuration
              }
            }
            addLog(
              scan,
