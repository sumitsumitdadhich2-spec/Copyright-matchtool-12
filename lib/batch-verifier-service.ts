...
const BATCH_VERIFY_MODEL_SPECS: Array<{ id: string; rpd: number }> = [
  { id: 'gemini-3.7-flash', rpd: 20 },
  { id: 'gemini-3.8-flash', rpd: 20 },
  { id: 'gemini-3.6-flash', rpd: 20 },
  { id: 'gemini-3.5-flash', rpd: 500 },
  { id: 'gemini-3.5-flash-lite', rpd: 500 },
...
/**
 * Gathers candidate lanes across all available API keys for all supported models.
 */
async function getBatchCandidateLanes(scan: Scan): Promise<CandidateLane[]> {
  ...
  allKeys.forEach((apiKey, keyIdx) => {
    for (const spec of BATCH_VERIFY_MODEL_SPECS) {
      if (!globalGeminiCoordinator.isModelExhausted(apiKey, spec.id, spec.rpd)) {
        lanes.push({
          apiKey,
          keyIdx: keyIdx + 1,
          modelId: spec.id,
